...
import io.streamthoughts.kafka.connect.filepulse.fs.BaseZipAmazonS3Test;
...
public class AmazonS3RowZipFileInputReaderTest extends BaseZipAmazonS3Test{
    ...
    public void setUp() throws Exception {
        ...
        ZipOutputStream zos =new ZipOutputStream(new FileOutputStream(objectFile.toPath().toString()));
        ZipEntry entry = new ZipEntry(objectFile.toPath().toFile().getName());
        entry.setCreationTime(FileTime.fromMillis(objectFile.toPath().toFile().lastModified()));
        entry.setComment("created by jimbo");
