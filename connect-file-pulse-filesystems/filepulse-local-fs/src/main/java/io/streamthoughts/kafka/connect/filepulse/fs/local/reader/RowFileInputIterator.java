...
import io.streamthoughts.kafka.connect.filepulse.fs.local.reader.internal.NonBlockingBufferReader;
import io.streamthoughts.kafka.connect.filepulse.fs.local.reader.internal.ReversedInputFileReader;
import io.streamthoughts.kafka.connect.filepulse.fs.local.reader.internal.TextBlock;
...
public class RowFileInputIterator extends AbstractFileInputIterator<TypedStruct> {
    ...
    private void mayReadHeaders() {
        ...
        final String path = file.getPath();
        if (skipHeaders > 0) {
            LOG.info("Starting to read header lines ({}) from file {}", skipHeaders, file.getName());
    ...
    public static class Builder {
        ...
        RowFileInputIterator build() {
            ...
            final File file = new File(context.metadata().uri());
            NonBlockingBufferReader reader = new NonBlockingBufferReader(file, initialBufferSize, charset);
